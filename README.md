...
- **การสำรองข้อมูลอัตโนมัติ** เก็บสำเนารายวันได้สูงสุด 30 ชุดไว้ที่
  `appdirs.user_data_dir("FuelTracker", "YourOrg")/backups/`
  (ดู [USAGE.md](docs/USAGE.md)).
...
ใช้คำสั่งต่อไปนี้ภายใต้สภาพแวดล้อมของ Poetry เพื่อรันทดสอบพร้อมวัดการครอบคลุม:

```bash
poetry run poe cover
```

หลังจากรันเสร็จ รายงานสรุปจะปรากฏบนหน้าจอและไฟล์ `.coverage` จะถูกสร้างขึ้น สามารถสร้างรายงานแบบ HTML เพื่อดูรายละเอียดได้ด้วย:

```bash
poetry run coverage html
```

แล้วเปิดไฟล์ `htmlcov/index.html` ในเบราว์เซอร์เพื่อดูผลลัพธ์
...
สร้างไฟล์สำรองด้วยตนเองได้ด้วยคำสั่ง

```bash
python -m fueltracker backup
```

คำสั่งนี้จะแสดงตำแหน่งไฟล์สำรองที่สร้างขึ้นบนหน้าจอ

และซิงก์ไฟล์สำรองขึ้นคลาวด์ (กำหนด `FT_CLOUD_DIR` ไว้ใน `.env` หรือ
ตัวแปรสภาพแวดล้อม) ได้ด้วย

```bash
python -m fueltracker sync
```
